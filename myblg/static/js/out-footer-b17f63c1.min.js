...
/*!
 * Generated using the Bootstrap Customizer (https://getbootstrap.com/customize/?id=c722d847877feddb1571)
...
function readyFn(jQuery){'use strict';(function(e,t,n,r){e.fn.doubleTapToGo=function(r){if(!("ontouchstart"in t)&&!navigator.msMaxTouchPoints&&!navigator.userAgent.toLowerCase().match(/windows phone os 7/i))return!1;this.each(function(){var t=!1;e(this).on("click",function(n){var r=e(this);if(r[0]!=t[0]){n.preventDefault();t=r}});e(n).on("click touchstart MSPointerDown",function(n){var r=!0,i=e(n.target).parents();for(var s=0;s<i.length;s++)if(i[s]==t[0])r=!1;if(r)t=!1})});return this}})(jQuery,window,document);jQuery('select').each(function(){if(jQuery(this).css('display')!=='none'){jQuery(this).parent().addClass('asalah_select_container')}});if(jQuery(window).width()>768){jQuery(document).on('mouseenter','.main_nav ul.navbar-nav > li, .main_nav ul.navbar-nav > li li',function(){var sub=jQuery(this).children('.dropdown-menu');if(sub.length>0){sub.stop().slideDown(200,'easeOutCubic')}});jQuery(document).on('mouseleave','.main_nav ul.navbar-nav > li, .main_nav ul.navbar-nav > li li',function(){var sub=jQuery(this).children('.dropdown-menu');if(sub.length>0){sub.stop().hide()}})}
...
var $originLeft=!0;if(jQuery('body.rtl').length===!0){$originLeft=!1}
var $blogisotope=jQuery('.blog_posts_wrapper.masonry_blog_style:not(.no_masonry_effect), body.paged .blog_posts_wrapper.banner_grid_blog_style').isotope({itemSelector:'.blog_post_container',layoutMode:'packery',transformsEnabled:!0,originLeft:$originLeft});document.addEventListener('lazybeforeunveil',function(){$blogisotope.imagesLoaded(function(){$blogisotope.isotope('layout')})});if(!jQuery('body').hasClass('paged')&&jQuery('.blog_posts_wrapper.banner_grid_blog_style').length!=='undefined'){var $gridblogisotope=jQuery('.blog_posts_wrapper.banner_grid_blog_style').imagesLoaded(function(){var column=jQuery('.blog_posts_wrapper.banner_grid_blog_style').width();if(jQuery('body').width()>900&&jQuery('.main_content').hasClass('col-md-12')){column=jQuery('.blog_posts_wrapper.banner_grid_blog_style').width()/3}else if((jQuery('body').width()<=900&&jQuery('body').width()>490)||(jQuery('body').width()>900&&jQuery('.main_content').hasClass('col-md-9'))){column=jQuery('.blog_posts_wrapper.banner_grid_blog_style').width()/2}
$gridblogisotope.isotope({itemSelector:'.blog_post_container',layoutMode:'packery',masonry:{columnWidth:column},transformsEnabled:!1,originLeft:$originLeft})})}
jQuery(window).on('resize load',function(){if(typeof $gridblogisotope!=='undefined'){$gridblogisotope.isotope('layout')}
if(typeof $blogisotope!=='undefined'){$blogisotope.isotope('layout')}});
...
$this.wrap('<div class="fluid-width-video-wrapper"></div>').parent('.fluid-width-video-wrapper').css('padding-top',(aspectRatio*100)+'%');$this.removeAttr('height').removeAttr('width')})})};$.fn.fitVids._count=0})(window.jQuery||window.Zepto);if(jQuery('.lazyload').length>0){jQuery('.video_frame.lazyload').on('lazyloaded',function(){jQuery(this).attr('src',jQuery(this).attr('data-src'));jQuery(this).removeAttr('data-src');jQuery('.video_fit_container').not('.filterable_grid .video_fit_container, .masonry_blog_style .video_fit_container').fitVids()})}else{jQuery('.video_fit_container').not('.filterable_grid .video_fit_container, .masonry_blog_style .video_fit_container').fitVids()};
